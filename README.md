...
```bash
bud-get$ sam build --use-container
```
...
```bash
bud-get$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
bud-get$ sam local start-api
bud-get$ curl http://localhost:3000/
```
...
```bash
bud-get$ sam logs -n HelloWorldFunction --stack-name bud-get --tail
```
...
```bash
bud-get$ pip install -r tests/requirements.txt --user
# unit test
bud-get$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
bud-get$ AWS_SAM_STACK_NAME="bud-get" python -m pytest tests/integration -v
```
